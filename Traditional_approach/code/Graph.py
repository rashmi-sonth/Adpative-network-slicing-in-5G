...
class Graph:
    def __init__(self, base_stations, clients, xlim, map_limits,
        ...
        # Predefined distinct colors for base stations
        colors = ['#1f77b4',  # Blue
          '#ff7f0e',  # Orange
          '#2ca02c',  # Green
          '#d62728',  # Red
        ...
        # Assign predefined colors to base stations
    ...
    def draw_map(self):
        ...
        for c in self.clients:
            label = None
            if c.subscribed_slice_index not in legend_labels and c.base_station is not None:
                label = c.get_slice().name
                legend_labels[c.subscribed_slice_index] = (label, c.base_station.color)

            self.ax.scatter(
                c.x, c.y,
                color=c.base_station.color if c.base_station is not None else '0.8',
                label=None,  # We handle legends separately below
                s=15,
        ...
        handles = []
        for index, (label, color) in legend_labels.items():
            handles.append(plt.Line2D([0], [0], marker=markers[index % len(markers)], color=color,
        ...
        leg = self.ax.legend(handles=handles, loc='upper center', bbox_to_anchor=(0.5, -0.05),
                            shadow=True, ncol=5, title="Slices")
        leg.set_title("Slices")
